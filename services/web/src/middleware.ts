...
const AUTH_COOKIES = ['authjs.session-token', 'next-auth.session-token', '__Secure-next-auth.session-token'];
...
export async function middleware(request: NextRequest) {
  ...
  const host = request.headers.get('X-Request-Domain') || request.headers.get('x-forwarded-host') || request.headers.get('host') || request.nextUrl.hostname;
