...
import type { UserListQuestionv1_0_0, StudentSubmission } from "@/interface-base/user.list.question.v1.0.0"
import FileExplorer from "./FileExplorer" // Import FileExplorer component
import type { CodeQuestionv1_0_0 } from "@/interface-base/code.question.v1.0.0" // Import CodeQuestionv1_0_0
...
const saveStateToLocalStorage = (
  ...
  if (!courseId || !moduleId || !assessmentId || !state.codeQuestion) return
...
interface CodeEditorPanelProps {
  codeQuestion: QuestionBasev1_0_0
  onReturn: () => void
...
export default function CodeEditorPanel({ codeQuestion, onSubmit, hierarchy = {
  ...
  const initialIsSubmitHidden = codeQuestion.rules.includes("submit: n")
  ...
  const maxChars =
    codeQuestion.rules && codeQuestion.rules.find((rule) => rule.startsWith("chars:"))
      ? Number.parseInt(
          codeQuestion.rules
  ...
  const maxFiles =
    codeQuestion.rules && codeQuestion.rules.find((rule) => rule.startsWith("files:"))
      ? Number.parseInt(
          codeQuestion.rules
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      // Remover a inicialização de codeFiles
      // setCodeFiles(loadedCodeFiles)
      ...
      if (codeQuestion.type === "code") {
        const codeQuestion = codeQuestion as CodeQuestionv1_0_0
  ...
  }, [codeQuestion, courseId, moduleId, assessmentId])

  useEffect(() => {
    const updateCharCounts = () => {
      const newTotalChars = calculateTotalChars(globalCodeFiles, codeQuestion.rules)
  ...
  }, [globalCodeFiles, codeFiles, activeFileIndex, codeQuestion.rules])

  useEffect(() => {
    const fetchData = async () => {
      if (submissionId) {
        try {
          const response = await fetch(`/api/teach/question/${codeQuestion.id}?courseId=${courseId}`)
          if (!response.ok) {
        ...
        try {
          ...
          if (codeQuestion.rules.includes("outputs: y") && !newAbortController.signal.aborted) {
            const results = compareOutputs([capturedOutput.join("\n")], {
              [currentFile.id]: codeQuestion.outputs[currentFile.id] || [],
            })
  ...
  const handleConfirmSubmit = () => {
    ...
    } as unknown as QuestionSubmission)
  ...
  const handleExportOpenFiles = async () => {
    ...
    if (folder) {
      ...
      a.href = url
      a.download = `${codeQuestion.title}_open_files.zip`
      a.click()
  ...
  const handleExportFiles = async (exportAll: boolean, singleFileName?: string) => {
    ...
    if (folder) {
      ...
      const a = document.createElement("a")
      a.href = url
      a.download = exportAll ? `${codeQuestion.title}_all_files.zip` : `${codeQuestion.title}_${singleFileName}.zip`
      a.click()
      window.URL.revokeObjectURL(url)
    }
  }
  ...
  }, [codeQuestion, codeFiles, globalCodeFiles, output, testResults, activeFileIndex, courseId, moduleId, assessmentId])
        ...
        >
          <div className="flex flex-col h-full">
            <LessonPanel
              ...
              isSubmitHidden={codeQuestion.rules.includes("submit: n")} // Updated line
              hierarchy={hierarchy}
              globalCodeFiles={globalCodeFiles} // Pass globalCodeFiles as a prop
              handleOpenFile={handleOpenFile}
              codeFiles={codeFiles} // Add this line
            />
          </div>
        </Panel>
        <PanelResizeHandle
            ...
            <Panel minSize={50}>
              <div className="flex flex-col h-full">
                <TopMenu
                  ...
                  onExportOpenFiles={handleExportOpenFiles} // Add this line
                  ...
                  onImportFolder={onImportFolder} // Added onImportFolder prop
                />
            ...
            <Panel defaultSize={25} minSize={0}>
              ...
              <div className="flex flex-col h-full">
                <BottomMenu
                  ...
                  mode={mode}
                  hideTestOutputs={codeQuestion.rules.includes("tests-outputs: n")}
                />
...
const ProblemsTab = ({ codeQuestion, mode }: { codeQuestion: QuestionBasev1_0_0; mode: string }) => {
  const hideTestOutputs = codeQuestion.rules.includes("tests-outputs: n")

  const renderInputs = () => {
    if (Array.isArray(codeQuestion.inputs)) {
      return (
    ...
    } else if (typeof codeQuestion.inputs === "object") {
      return (
        <ul className="list-disc list-inside">
          {Object.entries(codeQuestion.inputs).map(([key, value], index) => (
            <li
              key={index}
              className={mode === "light" ? "text-gray-800" : mode === "dark" ? "text-gray-200" : "text-white"}
            >
    ...
    <div>
      ...
      {codeQuestion.rules.includes("outputs: y") && !hideTestOutputs && (
        <div>
          ...
          <ul className="list-disc list-inside">
            {Object.entries(codeQuestion.outputs).map(([key, value], index) => (
              <li
                key={index}
                className={mode === "light" ? "text-gray-800" : mode === "dark" ? "text-gray-200" : "text-white"}
              >
...
const OutputTab = ({ output, mode }: { output: string; mode: string }) => {
  ...
  useEffect(() => {
    ...
    const svgMatch = output.match(/<svg.*?<\/svg>/s)
    if (svgMatch) {
...
const TestResultsTabComponent = ({ testResults, mode, hideTestOutputs, codeFiles }: TestResultsTabProps) => {
  const getColorClass = (passed: boolean) => {
    if (mode === "light") {
      return passed ? "bg-green100 text-green-800" : "bg-red-100 text-red-800"
    } else if (mode === "dark") {
    ...
    <div>
      <h2
        className={`text-xl font-bold mb-4 ${
          mode === "light" ? "text-gray-800" : mode === "dark" ? "text-gray-200" : "text-yellow-300"
        }`}
      >
        Test Results
      </h2>
      {Object.entries(groupedResults).length > 0 ? (
            ...
            <div key={fileId} className="mb-6">
              <h3
                className={`text-lg font-semibold mb-2 ${
                  mode === "light" ? "text-gray-700" : mode === "dark" ? "text-gray-300" : "text-yellow-200"
                }`}
              >
                File {codeFiles.findIndex((file) => file.id === fileId) + 1}
              </h3>
        ...
        <p className={mode === "light" ? "text-gray-600" : mode === "dark" ? "text-gray-400" : "text-yellow-200"}>
          No test results available. Run your code to see the results.
        </p>
      )}
    </div>
  )
