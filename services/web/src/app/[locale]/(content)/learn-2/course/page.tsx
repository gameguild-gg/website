...
import type { ModuleBasev1_0_0 } from "@/interface-base/module.base.v1.0.0"
import type { AssessmentBasev1_0_0 } from "@/interface-base/assessment.base.v1.0.0"
import type { CourseBasev1_0_0 } from "@/interface-base/course.base.v1.0.0"
import type { LessonBasev1_0_0 } from "@/interface-base/lesson.base.v1.0.0"
                ...
                {module.idLessons.map((lessonId) => {
                  ...
                  return lesson ? (
                    <div key={lesson.id} className="border rounded-md">
                      <Link
                        href={`/lesson/${lesson.id}?userId=${userId}&role=${role}&courseId=${params.id}&moduleId=${module.id}`}
                      >
                ...
                {module.idAssessments.map((assessmentId) => {
                  ...
                  return (
                    <div key={assessment.id} className="border rounded-md flex">
                      <Link
                        href={`/assessment/${assessment.id}?userId=${userId}&role=${role}&courseId=${params.id}&moduleId=${module.id}`}
                        className="grow"
                      >
                        <div className="w-full text-left px-4 py-2 flex items-center justify-between hover:bg-gray-50">
                          <div className="flex items-center">
