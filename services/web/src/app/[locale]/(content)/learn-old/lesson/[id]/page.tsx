...
import {
  LessonUpdateRequestv1_0_0,
  LessonUpdateResponsev1_0_0,
} from '@/lib/interface-base/lesson.update.v1.0.0';
...
export default function LessonPage({ params }: { params: { id: string } }) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-100 p-8">Loading lesson...</div>
    );
  }
  ...
  if (!lesson) {
    return (
      <div className="min-h-screen bg-gray-100 p-8">Lesson not found.</div>
    );
  }
  ...
  const renderContent = (
    ...
    if (typeof contentItem === 'string' || 'content' in contentItem) {
      const content =
        typeof contentItem === 'string' ? contentItem : contentItem.content;
      return (
        <div>
          {role === 'teacher' && !isEditing && (
            <Button onClick={() => handleEdit(index)} className="mb-2">
              Edit
            </Button>
          )}
          <Card>
              ...
              {isEditing ? (
                <div>
                  ...
                  <div className="mt-2 space-x-2">
                    <Button
                      onClick={() => handleSave(index, content, true)}
                      variant="default"
                    >
                      Save
                    </Button>
            ...
            <CardContent className="p-6">
              {isEditing ? (
                <div>
                  ...
                  <div className="mt-2 space-x-2">
                    <Button
                      onClick={() => handleSave(index, contentItem, true)}
                      variant="default"
                    >
                      Save
                    </Button>
