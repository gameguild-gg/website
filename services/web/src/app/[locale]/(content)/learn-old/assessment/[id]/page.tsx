...
export default function AssessmentPage({ params }: { params: { id: string } }) {
  const [assessment, setAssessment] = useState<AssessmentBasev1_0_0 | null>(
    null,
  );
  const [allQuestions, setAllQuestions] = useState<QuestionTypev1_0_0[]>([]);

  const [currentTabQuestions, setCurrentTabQuestions] = useState<
    QuestionTypev1_0_0[]
  >([]);

  const [selectedQuestion, setSelectedQuestion] =
    useState<QuestionTypev1_0_0 | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const assessmentResponse = await fetch(
        `../../../api/learn/content/assessment/${params.id}`,
      );
      if (!assessmentResponse.ok) {
        throw new Error(
          `Failed to fetch assessment: ${assessmentResponse.statusText}`,
        );
      }
      const assessmentData: AssessmentBasev1_0_0 =
        await assessmentResponse.json();
      setAssessment(assessmentData);

      const uniqueQuestionIds = Array.from(
        new Set(assessmentData.idQuestions.flatMap((tab) => tab.questions)),
      );
      ...
      const questionPromises = uniqueQuestionIds.map((id: number) =>
        fetch(`../../../api/learn/content/question/${id}`).then((res) => {
          if (!res.ok) {
            throw new Error(
              `Failed to fetch question ${id}: ${res.statusText}`,
            );
          }
          return res.json();
        }),
      );
      const allQuestionData: QuestionTypev1_0_0[] =
  ...
  const updateCurrentTabQuestions = (
    assessmentData: AssessmentBasev1_0_0,
    allQuestions: QuestionTypev1_0_0[],
    tabIndex: number,
  ) => {
    const currentTabQuestionIds =
      assessmentData.idQuestions[tabIndex].questions;
    const currentTabQuestions = currentTabQuestionIds
      .map((id) => allQuestions.find((q) => q.id === id))
      .filter((q): q is QuestionTypev1_0_0 => q !== undefined);
  ...
  const handleQuestionSelect = (question: QuestionTypev1_0_0) => {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-100 p-8">
        ...
        <div className="mt-4">
          <Link
            href={`/learn/course/${courseId}?userId=${userId}&role=${role}`}
          >
  ...
  if (!assessment) {
    return (
      <div className="min-h-screen bg-gray-100 p-8">Assessment not found.</div>
    );
  }
  ...
  const totalScore = assessment.idQuestions.reduce(
    (sum, tab) =>
      sum +
      tab.questions.reduce((tabSum, qId) => {
  ...
  const totalCurrentScore = assessment.idQuestions.reduce(
    (sum, tab) =>
      sum +
      tab.questions.reduce((tabSum, qId) => {
        const question = allQuestions.find((q) => q.id === qId);
  ...
  const totalQuestions = assessment.idQuestions.reduce(
    (sum, tab) => sum + tab.questions.length,
    0,
  );
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-700 high-contrast:bg-black p-4 rounded-lg shadow-md mb-8">
          <p className="text-lg dark:text-gray-300 high-contrast:text-yellow-300 mb-4">
          ...
          <p className="text-gray-700 dark:text-gray-300 high-contrast:text-yellow-300">
            Subjects:{' '}
            {Array.from(new Set(allQuestions.flatMap((q) => q.subject))).join(
              ', ',
            )}
          </p>
              ...
              <ul className="list-disc list-inside">
                {assessment.scoreTotal.map((score, index) => {
                  const discountedScore =
                    totalScore * (score.discount[0] / 100);
                  const scoreLoss = totalScore - discountedScore;
                  return (
                    <li
                      key={index}
                      className="text-gray-700 dark:text-gray-300 high-contrast:text-yellow-300 mb-2"
                    >
                      {new Date(score.maxDate).toLocaleDateString()}:
                      <span className="font-semibold">
                        {' '}
                        {discountedScore.toFixed(2)}{' '}
                      </span>
                      (Discount: {score.discount[1]}%, Score loss:
                      <span className="font-semibold text-red-500 dark:text-red-400 high-contrast:text-red-300">
                        {' '}
                        {scoreLoss.toFixed(2)}
                      </span>
                      )
        ...
        <div
          className={`flex flex-col md:flex-row gap-8 bg-white dark:bg-gray-800 high-contrast:bg-black rounded-b-lg shadow-lg overflow-hidden border-t border-gray-200 dark:border-gray-600 high-contrast:border-yellow-300`}
        >
          ...
          <div className="w-full md:w-2/3 p-8">
            {selectedQuestion ? (
              ...
              >
                <p>
                  Select a question to view its details and submit your answer.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="mt-8">
          ...
          >
            <button
              className={`w-full md:w-auto font-bold py-2 px-4 rounded transition-colors duration-200 ${
                mode === 'light'
