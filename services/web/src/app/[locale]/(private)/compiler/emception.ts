...
class Emception {
  ...
  async init() {
    console.log('inside init'); // this is not being print
  ...
  onprocessstart = (str) => {
    console.log('onprocessstart', str);
  };
  onprocessend = (str) => {
    console.log('onprocessend', str);
  };
  onstdout = (str) => {
    console.log(str);
  };
  ...
  run(...args) {
    ...
    return this._run_process_impl(
      [`/emscripten/${args[0]}.py`, ...args.slice(1)],
      {
  ...
  _run_process_impl(argv, opts: any = {}) {
    ...
    if (Array.isArray(argv[0])) {
      argv = argv[0];
      console.warn(
        'argv[0] is an array, recreating the command line arguments. FIXME!',
      ); // todo: fix this
    }
    ...
    const emscripten_script = argv0?.match(
      /^((\/lazy)?\/emscripten\/.+?)(?:\.py)?$/,
    )?.[1];
    console.log('emscripten_script', emscripten_script);
    if (
      emscripten_script &&
      this.fileSystem.exists(`${emscripten_script}.py`)
    ) {
      argv = [
        '/usr/bin/python',
        '-E',
    ...
    console.log(
      `Emception tool ${tool_name} finished with ${result.returncode}`,
    );
