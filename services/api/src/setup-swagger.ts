...
async function fixOpenApiGeneratorPlus() {
  await replaceInFile(process.cwd() + '/../../packages/apiclient/runtime.ts', 'import "whatwg-fetch";', '');
  await replaceInFile(process.cwd() + '/../../packages/apiclient/runtime.ts', 'window.fetch', 'fetch');
...
async function SetupDevelopmentConfigs(document: OpenAPIObject) {
  ...
  try {
    ...
    logger.error('Error generating client. Do you have installed java runtime?');
  }
...
export function setupSwagger(app: INestApplication): void {
  const documentBuilder = new DocumentBuilder().setTitle('gameguild.gg').addBearerAuth();
