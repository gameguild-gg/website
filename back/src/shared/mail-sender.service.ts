...
import {
    InternalServerErrorServerMisconfigurationException
} from "../exceptions/internal-server-error-server-misconfiguration.exception";
...
export class MailSenderService {
    ...
    async sendEmailVerification(user: {
        email: string,
    }, token: TokenPayloadDto, returnUrl: string) {
        if(!this.isSetup)
            throw new InternalServerErrorServerMisconfigurationException("SendGrid API Key not found in environment variables");

        var hostFront = this.configService.get<string>('HOST_FRONT_URL',"http://localhost:8080/verify-email");

        var url = hostFront + returnUrl + token.accessToken;

        var mail: MailDataRequired = {
            to: user.email,
