...
struct Board {
    ...
    vector<Position> NeighborsInsideBoundariesNotBlocked(const Position& p) const{
    ...
    bool CatCanMoveToPosition(const Position& move) const{
        unordered_set<Position> possibleMoves;
    ...
    float distanceToBorder(Position p){
...
vector<Position> buildPath(Board& board){
    vector<Position> path; // to be built and returned
    priority_queue<AStarNode> frontier; // the elements that will be explored
    unordered_map<Position, Position> cameFrom; // the flow field. if have the key, it means it is visited
...
Turn getTurn(const string& turn){
    if(turn == "CAT")
        return Turn::CAT;
