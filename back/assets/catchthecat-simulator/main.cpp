...
int main() {
    ...
    if(turn == "CAT") {
        ...
        Position catMove = cat.move(blocked, {catX, catY}, sideSize);
        auto elapsed = std::chrono::high_resolution_clock::now() - start;
        if(CatWon(Board(blocked, sideSize), catMove))
        ...
        Catcher catcher;
        auto start = std::chrono::high_resolution_clock::now();
        Position catcherMove = catcher.move(blocked, {catX, catY}, sideSize);
        auto elapsed = std::chrono::high_resolution_clock::now() - start;
        blocked[(catcherMove.y + sideSize/2) * sideSize + catcherMove.x+sideSize/2] = true;
        if(CatcherWon(Board(blocked, sideSize), catcherMove))
