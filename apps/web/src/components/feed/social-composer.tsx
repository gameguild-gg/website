...
import { createSocialPost, getSocialMediaStatus } from "@/lib/feed/actions";
...
export function SocialComposer({ userName, onPublished }: { userName: string; onPublished?: (post: SocialPostItem) => void }): React.JSX.Element {
  ...
  async function publish(event?: React.FormEvent) {
    ...
    try {
      ...
      const post = await createSocialPost({ content: text, visibility: "Public", assetReferenceId, tags: tagsFrom(text) });
      if (!publishedIdsRef.current.has(post.id)) { publishedIdsRef.current.add(post.id); onPublished?.(post); }
      setContent(""); setMedia(null); setProgress(null); setPhase("idle"); setExpanded(false); toast.success("Post published.");
    } catch (error) {
