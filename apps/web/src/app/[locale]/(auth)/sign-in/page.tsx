...
export default async function Page({
  ...
  const discordHref =
    '/api/auth/signin/discord?redirectTo=' +
    encodeURIComponent(`/${locale}${redirectTo}`) +
    `&locale=${locale}`;
      ...
      <SignInForm
        providers={
          <div className="flex flex-col gap-3">
            ...
            <a
              href={discordHref}
              data-testid="discord-sign-in-button"
              className={cn(
                buttonVariants({ variant: 'outline', size: 'lg', className: 'w-full' }),
              )}
            >
              <svg viewBox="0 0 24 24" fill="#5865F2" aria-hidden="true" className="size-5">
