...
public class RequireCommentPermissionAttribute : Attribute, IAsyncAuthorizationFilter
{
    ...
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        var hasPermission = await permissionService.HasResourcePermissionAsync<CommentPermission, cms.Common.Entities.Comment>(userId, tenantId, resourceId, _requiredPermission);
        if (!hasPermission)
        {
