...
{
    private const ResourceUsageType ProductResourceType = (ResourceUsageType)31;

    [Fact]
    public async Task UnknownProductCounter_IsNotInterpretedAsZero()
    {
        await using var db = CreateContext();
        db.Add(new ResourceQuota { TenantId = Guid.NewGuid(), Type = ProductResourceType, CurrentUsage = 7 });
    ...
    public async Task ProductAdapter_RepairsTenantPoolOnce_WithoutChangingUserQuota()
    {
        ...
        db.AddRange(
            new ResourceQuota { TenantId = tenantId, Type = ProductResourceType, CurrentUsage = 0 },
            new ResourceQuota { TenantId = tenantId, UserId = Guid.NewGuid(), Type = ProductResourceType, CurrentUsage = 9 });
        await db.SaveChangesAsync();
    ...
    {
        public ResourceUsageType ResourceType => ProductResourceType;
