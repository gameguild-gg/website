...
public sealed class GameGuildArchitectureAnalyzerTests
{
    ...
    [Fact]
    public async Task CommandCarryingExecutableDelegate_IsRejected()
    {
        const string source = """
            using System;
            namespace GameGuild.CQRS { public interface ICommand<T> { } }
            public sealed record ExecuteInControllerCommand(Func<int> Execute)
                : GameGuild.CQRS.ICommand<int>;
            """;

        var diagnostics = await AnalyzeAsync(source);

        diagnostics.Should().ContainSingle(diagnostic =>
            diagnostic.Id == GameGuildArchitectureAnalyzer.ExecutableDelegateCommandId);
    }

    [Fact]
    public async Task ControllerPersistenceHiddenBehindHelper_IsRejected()
    {
        const string source = """
            using System;
            namespace Microsoft.AspNetCore.Mvc { public sealed class HttpPostAttribute : Attribute { } }
            namespace GameGuild.CQRS
            {
                ...
                public interface ISender { T Send<T>(IRequest<T> command); }
            }
            public sealed class DbSession { public void SaveChangesAsync() { } }
            public sealed record ChangeNameCommand : GameGuild.CQRS.ICommand<bool>;
            public sealed class CustomerController(GameGuild.CQRS.ISender sender, DbSession db)
            {
                [Microsoft.AspNetCore.Mvc.HttpPost]
                public void Mutate()
                {
                    sender.Send(new ChangeNameCommand());
                    Persist();
                }

                private void Persist() => db.SaveChangesAsync();
            }
            """;
        ...
        diagnostics.Should().ContainSingle(diagnostic =>
            diagnostic.Id == GameGuildArchitectureAnalyzer.ControllerPersistenceBypassId);
    }
