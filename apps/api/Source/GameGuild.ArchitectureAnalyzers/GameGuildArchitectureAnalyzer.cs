...
public sealed class GameGuildArchitectureAnalyzer : DiagnosticAnalyzer
{
    ...
    private static bool SendsCommandArgument(InvocationExpressionSyntax invocation, SemanticModel semanticModel)
    {
        ...
        var argumentType = semanticModel.GetTypeInfo(argument.Expression).Type as INamedTypeSymbol;
        return argumentType is not null
               && (IsCommandInterface(argumentType) || argumentType.AllInterfaces.Any(IsCommandInterface));
    }
}
