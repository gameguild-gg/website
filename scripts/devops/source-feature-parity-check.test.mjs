...
test('shared host integration tests are blocking across product-specific test roots', (t) => {
  const f = fixture(t);
  fs.writeFileSync(f.policy, JSON.stringify({ schemaVersion: 1, requiredCommonModules: ['SharedKernel'],
    productModules: {}, testAliases: {},
    requiredCommonHostIntegrationTestPaths: ['DurableEventTransportPostgreSqlTests.cs'] }));
  f.put(0, 'apps/api/Tests/ModuEstate.API.IntegrationTests/ModuEstate.API.IntegrationTests.csproj', '<Project />');
  f.put(1, 'apps/api/tests/GameGuild.API.IntegrationTests/GameGuild.API.IntegrationTests.csproj', '<Project />');
  f.put(0, 'apps/api/Tests/ModuEstate.API.IntegrationTests/DurableEventTransportPostgreSqlTests.cs',
    'namespace ModuEstate; // preserves per-consumer inbox isolation');
  f.put(1, 'apps/api/tests/GameGuild.API.IntegrationTests/DurableEventTransportPostgreSqlTests.cs',
    'namespace GameGuild; // missing isolation assertion');

  const changed = f.run();

  assert.equal(changed.status, 1);
  const area = changed.report.areas.find(candidate => candidate.kind === 'shared-host-integration-test');
  assert.equal(area.counts.different, 1);
});
