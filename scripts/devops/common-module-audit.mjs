...
function checkPolicy(policy) {
  for (const hostPaths of [
    policy.requiredCommonHostPaths ?? [],
    policy.requiredCommonHostTestPaths ?? [],
    policy.requiredCommonHostIntegrationTestPaths ?? [],
  ]) {
...
export function audit({ root, peerRoot, policy, scope = 'api', baseline }) {
  ...
  for (const name of policy.requiredCommonHostIntegrationTestPaths ?? []) {
    const roots = repos.map((repo, index) =>
      `${tests[index].get('API.IntegrationTests') ?? `apps/api/Tests/${repo.brand}.API.IntegrationTests`}/${name}`);
    const presence = repos.map((repo, index) => [...repo.files.keys()].some(file => file === roots[index] || file.startsWith(`${roots[index]}/`)));
    if (!presence.every(Boolean))
      report.issues.push({ code: 'missing-common-host-integration-test-path', name, missingIn: brands.filter((_, index) => !presence[index]) });
    report.areas.push(compare(repos, 'shared-host-integration-test', name, roots));
  }
  if (scope === 'all') {
